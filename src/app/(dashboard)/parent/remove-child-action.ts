...
export async function removeChildAction(studentId: string) {
  ...
  if (count && count > 0) {
    return { error: 'Cannot remove a child with active enrollments. Drop all classes first.' };
  }
  ...
  if (waitlistCount && waitlistCount > 0) {
    return { error: 'Cannot remove a child who is on a waitlist.' };
  }

  const childUserId = student.user_id;
  ...
  const { error: authErr } = await adminClient.auth.admin.deleteUser(childUserId);
