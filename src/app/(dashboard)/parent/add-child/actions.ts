...
export async function addChildAction(formData: FormData) {
  ...
  if (!fullName) return { error: 'Child name is required.' };
  ...
  // Prevent duplicate: check if parent already has a child with this exact name
  {
    const { data: existingChildren, error: childErr } = await adminSupabase
      .from('students')
      .select('id, full_name')
    ...
    if (!childErr) {
      ...
      if (duplicate) {
        return { error: 'A child with this name already exists.' };
      }
    }
  }
