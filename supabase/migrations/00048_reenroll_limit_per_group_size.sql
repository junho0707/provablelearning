-- Fix: re-enrollment limit is now per (course + group_size_type).
-- A student taking "DSAT RW Advanced - Small" 3 times can still enroll in "DSAT RW Advanced - Medium".
-- Counts only active + completed enrollments (refunded/canceled don't count).
...
DECLARE
  ...
  v_count INTEGER;
  v_group_size_type group_size_type;
BEGIN
  ...
  -- Get the group_size_type of the class being enrolled into
  SELECT c.group_size_type
  INTO v_group_size_type
  FROM classes c
  ...
  WHERE e.student_id = NEW.student_id
    ...
    AND c.group_size_type::text = v_group_size_type::text
    AND e.status IN ('active', 'completed');

  IF v_count >= v_max_reenroll THEN
    RAISE EXCEPTION 'Student has reached re-enrollment limit (%) for this course and group size', v_max_reenroll;
  END IF;
...
BEGIN
  ...
  SELECT c.capacity, c.course_id, c.active, c.group_size_type, m.start_date, m.max_reenroll
  INTO v_capacity, v_class_course_id, v_class_active, v_group_size_type, v_course_start, v_max_reenroll
  FROM classes c
  ...
  -- 5. Check re-enrollment limit per course + group_size_type
  SELECT COUNT(*)
  ...
  WHERE e.student_id = p_student_id
    ...
    AND cl.group_size_type::text = v_group_size_type::text
    AND e.status IN ('active', 'completed');

  IF v_reenroll_count >= v_max_reenroll THEN
    RAISE EXCEPTION 'Re-enrollment limit reached (% of % for this course and group size)', v_reenroll_count, v_max_reenroll;
  END IF;
...
-- =============================================================================
-- 3. Fix auto_enroll_from_waitlist RPC
-- =============================================================================
...
BEGIN
  ...
  -- Lock and fetch class info (includes group_size_type)
  SELECT c.id, c.capacity, c.course_id, c.active, c.meeting_day, c.meeting_time, c.group_size_type,
  ...
  LOOP
    ...
    -- Check 3: re-enrollment limit per course + group_size_type
    SELECT COUNT(*) INTO v_reenroll_count
    FROM enrollments enr
    JOIN classes cl ON cl.id = enr.class_id
    WHERE enr.student_id = v_entry.student_id
      AND enr.course_id = v_class.course_id
      AND cl.group_size_type::text = v_class.group_size_type::text
