-- Global enrollment caps for SG & 1:1
-- Limits total students across all slots for a given subject+level+group_size combination.

CREATE TABLE public.enrollment_caps (
  ...
  UNIQUE (subject, level, group_size_type)
);

ALTER TABLE public.enrollment_caps ENABLE ROW LEVEL SECURITY;

CREATE POLICY enrollment_caps_select ON public.enrollment_caps
  FOR SELECT USING (true);

CREATE POLICY enrollment_caps_admin_all ON public.enrollment_caps
  FOR ALL USING ((SELECT role FROM users WHERE id = auth.uid()) = 'admin');

-- Updated reserve_seat with global enrollment cap check
