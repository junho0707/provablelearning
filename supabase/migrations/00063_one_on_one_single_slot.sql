-- 1:1 classes: enforce single slot only (1x/wk, 2hr sessions)
-- Re-create reserve_seat with guard against slot2 for 1:1
...
BEGIN
  ...
  -- Compute rolling dates (SG/1:1) or fixed dates (LG)
  IF v_slot1.group_size_type = 'large' THEN
