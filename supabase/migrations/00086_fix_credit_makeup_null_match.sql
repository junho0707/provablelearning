-- Migration 00086: Fix book_makeup_with_credit NULL subject/level matching
-- PostgreSQL: NULL = NULL → NULL (falsy), so SG/1:1 credits (where both
-- subject and level are NULL) never matched.  Use IS NOT DISTINCT FROM.

DROP FUNCTION IF EXISTS public.book_makeup_with_credit(UUID, UUID, DATE, UUID);
...
BEGIN
  ...
  --    Use IS NOT DISTINCT FROM for subject/level so NULL = NULL matches.
  SELECT * INTO v_credit
  FROM credits cr
  WHERE cr.student_id = p_student_id
    AND cr.group_size_type = v_class.group_size_type
    AND cr.subject IS NOT DISTINCT FROM v_class.subject
    AND cr.level IS NOT DISTINCT FROM v_class.level
